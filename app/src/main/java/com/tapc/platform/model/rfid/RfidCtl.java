...
import com.tapc.platform.model.rfid.usb.UsbCtl;
import com.tapc.platform.model.rfid.usb.UsbRecvListerner;
...
public class RfidCtl {
	...
	private void sendCardUid(byte[] datas) {
		if (datas[3] > 0) {
			...
			setCardUid(uid2);
			Log.d("detect card uid", "" + UsbCtl.bytesToHexString(uid2));
			if (mRfidListener != null) {
				mRfidListener.detectCard(getCardUid());
			}
		}
	}
